...
namespace DrawingStuff
{
    internal static class ResourceManager
    {
        ...
        private static Resource GetResource<Resource>(string filename, Dictionary<string, Resource> loadedResources, Func<string, Resource> loadResourceFunc)
        {
            if (!loadedResources.TryGetValue(filename, out Resource resource))
            {
        ...
        public static Texture2D LoadTexture(string filename)
        {
            try
            {
                Bitmap bitmap = new Bitmap(TexturesPath + filename);
